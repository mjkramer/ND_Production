...
@defplot('comparison of reco vs true event ID')
...
def main(flow_file, charge_only):
    ...
    with PdfPages(output_pdf_name) as output:
        ...
        if 'mc_truth' in flow_h5.keys():
            plot_packet_fractions_sum(flow_h5, output)
            plot_num_backtracked_segments(flow_h5, output)
            plot_true_vs_hit_positions(flow_h5, output)
